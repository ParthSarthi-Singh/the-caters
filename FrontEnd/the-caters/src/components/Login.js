...
export default function Login(props) {
    ...
    const loggedIn = async (e) => {
        ...
        if (json.auth) {
            ...
            props.setUserCreds(json.userCreds)
            navigate('/')
        }
        else {
            alert(json.msg)
                                        ...
                                        <div className="card-body p-4 p-lg-5 text-black">
                                            ...
                                            <form>
                                                ...
                                                <div className="pt-1 mb-4">
                                                    <button className="btn btn-dark" type="button" onClick={loggedIn} style={{ backgroundColor: "black" }}>Login</button>
                                                </div>
                                                <div className="pt-1 mb-4">
                                                    <Link className="btn btn-dark " to="/vendorLogin" role="button">Login as a Vendor</Link>
                                                </div>

                                                <Link className="small" to="/signup">Forgot password?</Link>

                                                <p className="mb-5 pb-lg-2 italic text-muted " style={{ cursor: "default" }} >Don't have an account? &ensp;
                                                    <Link to="/signup" style={{ color: "#393f81" }} className="text-decoration-underline" ><u>Register here</u> </Link></p>
                                            </form>
